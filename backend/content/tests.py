...
class AdminCustomizationTests(TestCase):
    ...
    def test_deployment_settings_admin_encrypts_smtp_password_and_shows_status(self):
        ...
        self.assertContains(response, "Allow EmailJS API for non-browser applications")
        self.assertContains(response, "注册 EmailJS 个人账号")
        self.assertContains(response, "Brevo 配置流程")
        self.assertContains(response, "SMTP 配置流程")
